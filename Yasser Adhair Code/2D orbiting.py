...
def update_frame(frame):
    global earth_v,earth_a,earth_mass,dt,earth_pos,earth_r,earth_theta
    global M,G
    ...
    earth_theta = find_theta(earth_pos)
    earth_a = update_acceleration(G,M,earth_mass,earth_r,earth_theta)
    earth.set_center(earth_pos)
    earth_line.set_data(earth_x,earth_y)

    global mercury_v,mercury_a,mercury_mass,mercury_pos,mercury_r,mercury_theta
    mercury_v = update_v(mercury_v,dt,mercury_a)
    mercury_pos = update_pos(mercury_pos,dt,mercury_v)
    mercury_x.append(mercury_pos[0])
    mercury_y.append(mercury_pos[1])
    mercury_r = find_r(mercury_pos)
    mercury_theta = find_theta(mercury_pos)
    mercury_a = update_acceleration(G,M,mercury_mass,mercury_r,mercury_theta)
    mercury.set_center(mercury_pos)
    mercury_line.set_data(mercury_x,mercury_y)

    global venus_v,venus_a,venus_mass,venus_pos,venus_r,venus_theta
    venus_v = update_v(venus_v,dt,venus_a)
    venus_pos = update_pos(venus_pos,dt,venus_v)
    venus_x.append(venus_pos[0])
    venus_y.append(venus_pos[1])
    venus_r = find_r(venus_pos)
    venus_theta = find_theta(venus_pos)
    venus_a = update_acceleration(G,M,venus_mass,venus_r,venus_theta)
    venus.set_center(venus_pos)
    venus_line.set_data(venus_x,venus_y)

    global mars_v,mars_a,mars_mass,mars_pos,mars_r,mars_theta
    mars_v = update_v(mars_v,dt,mars_a)
    mars_pos = update_pos(mars_pos,dt,mars_v)
    mars_x.append(mars_pos[0])
    mars_y.append(mars_pos[1])
    mars_r = find_r(mars_pos)
    mars_theta = find_theta(mars_pos)
    mars_a = update_acceleration(G,M,mars_mass,mars_r,mars_theta)
    mars.set_center(mars_pos)
    mars_line.set_data(mars_x,mars_y)

    global jupiter_v,jupiter_a,jupiter_mass,jupiter_pos,jupiter_r,jupiter_theta
    jupiter_v = update_v(jupiter_v,dt,jupiter_a)
    jupiter_pos = update_pos(jupiter_pos,dt,jupiter_v)
    jupiter_x.append(jupiter_pos[0])
    jupiter_y.append(jupiter_pos[1])
    jupiter_r = find_r(jupiter_pos)
    jupiter_theta = find_theta(jupiter_pos)
    jupiter_a = update_acceleration(G,M,jupiter_mass,jupiter_r,jupiter_theta)
    jupiter.set_center(jupiter_pos)
    jupiter_line.set_data(jupiter_x,jupiter_y)

    global saturn_v,saturn_a,saturn_mass,saturn_pos,saturn_r,saturn_theta
    saturn_v = update_v(saturn_v,dt,saturn_a)
    saturn_pos = update_pos(saturn_pos,dt,saturn_v)
    saturn_x.append(saturn_pos[0])
    saturn_y.append(saturn_pos[1])
    saturn_r = find_r(saturn_pos)
    saturn_theta = find_theta(saturn_pos)
    saturn_a = update_acceleration(G,M,saturn_mass,saturn_r,saturn_theta)
    saturn.set_center(saturn_pos)
    saturn_line.set_data(saturn_x,saturn_y)

    global uranus_v,uranus_a,uranus_mass,uranus_pos,uranus_r,uranus_theta
    uranus_v = update_v(uranus_v,dt,uranus_a)
    uranus_pos = update_pos(uranus_pos,dt,uranus_v)
    uranus_x.append(uranus_pos[0])
    uranus_y.append(uranus_pos[1])
    uranus_r = find_r(uranus_pos)
    uranus_theta = find_theta(uranus_pos)
    uranus_a = update_acceleration(G,M,uranus_mass,uranus_r,uranus_theta)
    uranus.set_center(uranus_pos)
    uranus_line.set_data(uranus_x,uranus_y)

    global neptune_v,neptune_a,neptune_mass,neptune_pos,neptune_r,neptune_theta
    neptune_v = update_v(neptune_v,dt,neptune_a)
    neptune_pos = update_pos(neptune_pos,dt,neptune_v)
    neptune_x.append(neptune_pos[0])
    neptune_y.append(neptune_pos[1])
    neptune_r = find_r(neptune_pos)
    neptune_theta = find_theta(neptune_pos)
    neptune_a = update_acceleration(G,M,neptune_mass,neptune_r,neptune_theta)
    neptune.set_center(neptune_pos)
    neptune_line.set_data(neptune_x,neptune_y)

    return earth, sun, earth_line, mercury, mercury_line, venus, venus_line, mars, mars_line, jupiter,jupiter_line, saturn,saturn_line, uranus, uranus_line, neptune, neptune_line


def init_func():
    ax.grid()
    ax.set_title('Solar System Orbit Simulation')
    ax.set_xlabel('Semi-major axis /AU')
    ax.set_ylabel('Semi-minor axis /AU')
    global earth, mercury, venus, mars, jupiter, saturn, sun, uranus, neptune
    earth = patches.Circle(earth_pos, radius = 0.08,fc= 'b')
    sun = patches.Circle(M_pos, radius= 0.1, fc= 'yellow')
    mercury = patches.Circle(mercury_pos, radius = 0.05, fc = 'r')
    venus = patches.Circle(venus_pos, radius =0.08, fc = 'orange')
    mars = patches.Circle(mars_pos, radius=0.1, fc = 'r')
    jupiter = patches.Circle(jupiter_pos, radius = 0.2, fc = 'brown')
    saturn = patches.Circle(saturn_pos, radius = 0.2, fc = 'sandybrown')
    uranus = patches.Circle(uranus_pos, radius = 0.2,fc= 'lightblue')
    neptune = patches.Circle(neptune_pos, radius = 0.2,fc= 'darkblue')
    ax.add_patch(earth)
    ax.add_patch(sun)
    ...
    ax.set_aspect('equal')
    global earth_line, mercury_line, venus_line, mars_line, jupiter_line, saturn_line, uranus_line, neptune_line
