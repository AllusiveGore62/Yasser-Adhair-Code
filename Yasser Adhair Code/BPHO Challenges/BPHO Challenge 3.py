...
def update_frame(frame):
    ...
    try: #Added try if i is out of range meaning at least one of the balls hit the desired point
        ...
        if x_pos_high[i] < highest_x: #high ball check
        ...
        if x_pos_min[i] < highest_x: #min ball check
        ...
        if x_pos_low[i] < highest_x: #low ball check
        ...
        return line_min, line_low, line_high, #Return the lines to the FuncAnimation function
    except IndexError:
        return line_min, line_low, line_high
        


def init_func(): 
    #This function is passed to the Func Animation function to initialise the graph
    global line_min, line_low, line_high, ax, root
...
def graph(X, Y_real, u, h):
    ...
    highest_x = X #We want the animations to stop when X is reached
    ...
    plt.axis([0,((1.2*highest_x)//1)+1,0,((1.1*highest_y_high)//1)+1]) #Setting axis range, using high ball for y limit
    plt.legend() 
